...
#if defined(_ESM) || defined(_VSM) || defined(_EVSM)
    TEXTURE2D_FLOAT(_MainLightShadowmapTextureEX);
    SAMPLER(sampler_MainLightShadowmapTextureEX_Clamp_Linear);
#endif
...
real SampleESMPreFilteredShadowmap(TEXTURE2D_PARAM(ShadowMap, sampler_ShadowMap), float4 shadowCoord, ShadowSamplingData samplingData)
{
    real attenuation = SAMPLE_TEXTURE2D(ShadowMap, sampler_ShadowMap, shadowCoord.xy).r;
    ...
    // e^(-cz) * e^(cd)
    attenuation = saturate(exp(-_ESMConst * shadowCoord.z) * attenuation);
    //attenuation = saturate(exp(attenuation - _ESMConst * shadowCoord.z));
#endif
...
inline real remapping(real shadow_factor, real minVal)
{
    return saturate((shadow_factor - minVal) / (1.0 - minVal));
}


inline real Chebyshev(real2 moments, real mean, real minVariance, real reduceLightBleeding)
{
    real variance = moments.y - (moments.x * moments.x);
    variance = max(variance, minVariance);
    real d = mean - moments.x;

    real pMax = remapping(variance / (variance + (d * d)), reduceLightBleeding);

#if UNITY_REVERSED_Z
    //return mean > moments.x ? 1 : pMax;
    real p = step(moments.x, mean);
    return max(p, pMax);
#else
    //return mean <= moments.x ? 1 : pMax;
    real p = step(mean, moments.x);
    return max(p, pMax);
#endif
}

// VSM F = v^2 / (v^2 + (z - d)^2)
real SampleVSMPreFilteredShadowmap(TEXTURE2D_PARAM(ShadowMap, sampler_ShadowMap), float4 shadowCoord, ShadowSamplingData samplingData)
{
    real depth = shadowCoord.z;
    real2 moments = SAMPLE_TEXTURE2D(ShadowMap, sampler_ShadowMap, shadowCoord.xy).xy;
    real p = Chebyshev(moments, depth, 0.0001f, _VSMMin);
...
inline real2 warpDepth(real depth)
{
    real pos = exp(_EVSMConstX * depth);
    real neg = -exp(-_EVSMConstY * depth);
    real2 wDepth = real2(pos, neg);
    return wDepth;
}

real SampleEVSMPreFilteredShadowmap(TEXTURE2D_PARAM(ShadowMap, sampler_ShadowMap), float4 shadowCoord, ShadowSamplingData samplingData)
{
    real4 moments = SAMPLE_TEXTURE2D(ShadowMap, sampler_ShadowMap, shadowCoord.xy);
    real2 positiveMoments = moments.xy;
    real2 negativeMoments = moments.zw;

    real2 depth = warpDepth(shadowCoord.z);
    real2 depthScale = real2(_EVSMConstX, _EVSMConstY) * depth;
    real2 minVariance = 0.0001f * depthScale * depthScale;
    real positiveResult = Chebyshev(positiveMoments, depth.x, minVariance.x, _EVSMMin);
    real negativeResult = Chebyshev(negativeMoments, depth.y, minVariance.y, _EVSMMin);

    real p = min(positiveResult, negativeResult);
    return p;
}


real SampleShadowmapEX(TEXTURE2D_PARAM(ShadowMap, sampler_ShadowMap), float4 shadowCoord, ShadowSamplingData samplingData, half4 shadowParams, bool isPerspectiveProjection = true)
{
    // Compiler will optimize this branch away as long as isPerspectiveProjection is known at compile time
    if (isPerspectiveProjection)
        shadowCoord.xyz /= shadowCoord.w;
...
#if defined(_ESM)
    attenuation = SampleESMPreFilteredShadowmap(ShadowMap, sampler_ShadowMap, shadowCoord, samplingData);
#elif defined(_VSM)
    attenuation = SampleVSMPreFilteredShadowmap(ShadowMap, sampler_ShadowMap, shadowCoord, samplingData);
#elif defined(_EVSM)
    attenuation = SampleEVSMPreFilteredShadowmap(ShadowMap, sampler_ShadowMap, shadowCoord, samplingData);
#endif
...
#if defined(_ESM) || defined(_VSM) || defined(_EVSM)
    return SampleShadowmapEX(TEXTURE2D_ARGS(_MainLightShadowmapTextureEX, sampler_MainLightShadowmapTextureEX_Clamp_Linear), shadowCoord, shadowSamplingData, shadowParams, false);
#else
